...
(function( $ ){
	...
	P3.initialize = function() {
		if ( P3StoredVersion < P3CurrentVersion ) {
			// P3 upgrades on admin_init.  If it needs it, send a noop admin-ajax call to trigger.
			$.ajax( ajaxUrl, {
				data: {
					action: 'P3.noop'
				},
		...
		// ------------------------------------------------
		P3.keyboard();
		P3.comment.highlight();
		P3.utility.localizeMicroformatDates();

		// Initialize mention autocompletes if mention data exists.
		if ( mentionData )
			P3.autocomplete.mentions( $( '#posttext, #comment' ) );

		// Activate Tag Suggestions for logged users on front page
		if (isFrontPage && prologueTagsuggest && isUserLoggedIn)
			P3.autocomplete.tags( $( 'input[name="tags"]' ) );

		// Turn on automattic updating
		if (prologuePostsUpdates)
			P3.utility.toggleUpdates('unewposts');
		if (prologueCommentsUpdates)
			P3.utility.toggleUpdates('unewcomments');

		// Activate autgrow on textareas
		P3.utility.autoGrowTextarea($('#posttext, #comment'));

		// Activate tooltips on recent-comments widget
		P3.utility.tooltip($("table.P3-recent-comments a.tooltip"));

		// Hide error messages on load
		$('#posttext_error, #commenttext_error').hide();

		// Bind actions to comments and posts
		$( 'body .post, body .page' ).each( function() { P3.utility.bindActions( this, 'post' ); } );
		$( 'body .comment' ).each( function() { P3.utility.bindActions( this, 'comment' ); } );
		...
		if	($("table.P3-recent-comments").length != 0)
			lcwidget = true;
		...
		$(window).bind('beforeunload', function(e) {
			if ( $( '#posttext' ).val() || $( '#comment' ).val() ) {
				...
				if (e) { // For IE and Firefox
					e.returnValue = P3txt.unsaved_changes;
				}
				return P3txt.unsaved_changes;   // For Safari
			}
		});
				...
				$.ajax({
					...
					success: function( response ) {
						if ( 'logged_in' != response ) {
							...
							P3.utility.notification( P3txt.oops_not_logged_in + ' <a href="' + login_url + '">' + P3txt.please_log_in + '</a>.', true );
					...
					error: function() {
						...
						P3.utility.notification( P3txt.whoops_maybe_offline );
						event.preventDefault();
						return false;
					}
				});
		...
		$(window).scroll(function() { P3.utility.removeYellow(); });
	...
	P3.comment = {
				...
				if (newComments != null) {
					$.each(newComments.comments, function(i,comment) {
						...
						if (comment.widgetHtml) {
							P3.comment.insert.widget(comment.widgetHtml);
						}
						if (comment.html != '') {
							var thisParentId = 'prologue-'+comment.postID;
							P3.comment.insert.inline(thisParentId, comment.commentParent, comment.html, showNotification);
						}
					});
					if (showNotification) {
						P3.utility.notification(P3txt.n_new_comments.replace('%d', newComments.numberofnewcomments));
					}
				}
			});
			P3.utility.toggleUpdates('unewcomments');
		...
		insert: {
			...
			inline: function( postParent, comment_parent, commentHtml, showNotification ) {
				...
				newCommentsLi.slideDown( 200, function() {
					...
					newComment.children('li.newcomment').each(function() {
						if (P3.utility.isElementVisible(this) && !showNotification) {
							$(this).animate({backgroundColor:'transparent'}, {duration: 1000}, function(){
								$(this).removeClass('newcomment');
							});
						}
						P3.utility.bindActions(this, 'comment');
					});
					P3.utility.localizeMicroformatDates(newComment);
			...
			widget: function( widgetHtml ) {
				$("table.P3-recent-comments").each(function() {
					var t = $(this);
					...
					P3.utility.tooltip($("tbody tr:first td a.tooltip", t));
					if (P3.utility.isElementVisible(newCommentsElement)) {
		...
		submit: function( trigger ) {
			...
			if ('' == commenttext) {
				$("label#commenttext_error").text(P3txt.required_field).show().focus();
				return false;
			}
			...
			// If P3 thinks current user is logged in, check again for being logged in,
			// and check for a working connection to the server, to avoid hanging comments.
			if ( isUserLoggedIn && ( 'logged_in' != P3.utility.loggedInOut() ) ) {
				submitProgress.hide();
			...
			$.ajax( {
				...
				success: function( result ) {
					...
					$form.slideUp( 200, function() {
						...
						$( '#comment' ).val( '' );
						if ( errorMessage != "" )
							P3.utility.notification( errorMessage );

						P3.comment.fetch( false );
			...
			document.onkeypress = function(e) {
				...
				switch(keyCode) {
					...
					case 67:
						...
						if (!P3.utility.isElementVisible("#postbox"))
							$.scrollTo('#postbox', 50);
						$("#posttext").focus();
						if (e.preventDefault)
							e.preventDefault();
						...
						if (!commentLoop) {
							if (currPost > 0) {
								...
								if (0 != $('#'+postsOnPage[currPost]).children('ul.commentlist').length && !hidecomments) {
									...
									if (!P3.utility.isElementVisible('#'+commentsOnPost[currComment]))
										$.scrollTo('#'+commentsOnPost[currComment], 150);
									return;
								}
								if (!P3.utility.isElementVisible('#'+postsOnPage[currPost]))
					...
					case 79:
						...
						// "t" key
					case 84:
						P3.utility.jumpToTop();
						break;
					// "esc" key
					case 27:
						if (element.tagName == 'INPUT' || element.tagName == 'TEXTAREA') {
							if ( e.shiftKey && confirm( P3txt.comment_cancel_ays ) ) {
	...
	P3.plugins = {
		fluid: function( value ) {
			...
			// Sets the badge for Fluid app enclosed P3. See http://fluidapp.com/
	...
	P3.posts = {
		...
		edit: function( postId, element ) {
			...
			$.getJSON( ajaxUrl, { action: 'get_post', _inline_edit: nonce, post_ID: 'content-' + postId }, function( post ) {
				...
				if (post.post_format == 'standard' || post.post_type == 'page') {
					...
					defaultText(titleInput, P3txt.title);
				...
				P3.utility.autoGrowTextarea($(editor));
				...
				if (post.post_format == 'quote') {
					...
					defaultText(citationInput, P3txt.citation);
				...
				if ( post.post_type != 'page' ) {
					...
					defaultText(tagsInput, P3txt.tagit);
				...
				saveButton.innerHTML = P3txt.save;
				$( saveButton ).click( function() {
					var tags = ! tagsInput || tagsInput.value == P3txt.tagit ? '' : tagsInput.value;
					var args = {
						action:'save_post',
						_inline_edit: nonce,
					...
					if (post.post_format == 'standard' || post.post_type == 'page') {
						args.title = titleInput.value == P3txt.title ? '' : titleInput.value;
					} else if (post.post_format == 'quote') {
						args.citation = citationInput.value == P3txt.citation ? '' : citationInput.value;
					}
				...
				cancelButton.innerHTML = P3txt.cancel;
				...
				// Initialize tags autocomplete
				if ( tagsInput )
					P3.autocomplete.tags( $( tagsInput ) );
			...
			ajaxCheckPosts = $.getJSON(queryString, function(newPosts){
				if (newPosts != null) {
					...
					if (!isFirstFrontPage || (typeof newPosts.html == "undefined") ) {
						...
						message = P3txt.n_new_updates.replace('%d', newUnseenUpdates) + " <a href=\"" + wpUrl +"\">"+P3txt.goto_homepage+"</a>";
						P3.utility.notification(message);
					} else {
						...
						newUpdatesLi.each(function() {
							...
							P3.utility.bindActions(this, 'post');
							if (P3.utility.isElementVisible(this) && !showNotification) {
							...
							P3.utility.localizeMicroformatDates(this);
							counter++;
						});
						if (counter >= newPosts.numberofnewposts && showNotification) {
							var updatemsg = P3.utility.isElementVisible('#main > ul >li:first') ? "" :  "<a href=\"#\"  onclick=\"P3.utility.jumpToTop();\" \">"+P3txt.jump_to_top+"</a>" ;
							P3.utility.notification(P3txt.n_new_updates.replace('%d', counter) + " " + updatemsg);
							P3.utility.titleCount();
						}
					}
					$('.newupdates > h4, .newupdates > div').hover( P3.utility.removeYellow, P3.utility.removeYellow );
				}
			});
		...
		submit: function( trigger ) {
			...
			if ("" == posttext) {
				$("label#posttext_error").text(P3txt.required_field).show().focus();
				return false;
			}
			...
			$.ajax({
				...
				success: function(result) {
					if ("0" == result)
						errorMessage = P3txt.not_posted_error;

					$('#posttext').val('');
					$('#posttitle').val('');
					$('#postcitation').val('');
					$('#tags').val(P3txt.tagit);
					if(errorMessage != '')
						P3.utility.notification(errorMessage);
					...
					if (isFirstFrontPage && result != "0") {
						P3.posts.fetch(false);
					} else if (!isFirstFrontPage && result != "0") {
						P3.utility.notification(P3txt.update_posted);
					}
	...
	P3.utility = {
		autoGrowTextarea: function( textareas ) {
			...
			textareas.on('focus blur keydown paste', function(e) {
				...
				if ( 'paste' === e.type )
					setTimeout( function() { P3.utility.expandTextarea( t, $('#' + $(t).prop('id')).prop('scrollHeight') ); }, 30 );
				else
					P3.utility.expandTextarea( t, scrollHeight );
			});
		},
		bindActions: function( element, type ) {
			...
			switch (type) {
				case "comment" :
					...
					$(element).hover( P3.utility.removeYellow, P3.utility.removeYellow );
					if (inlineEditComments != 0 && isUserLoggedIn) {
						...
						thisCommentEditArea.editable(ajaxUrl, {event: 'edit', loadurl: ajaxUrl + '&action=get_comment&_inline_edit=' + nonce,
							...
							indicator : commentEditSpinnerText, loadtext: P3txt.loading, cancel: P3txt.cancel,
							submit  : P3txt.save, tooltip   : '', width: '90%', onblur: 'ignore',
		...
		removeYellow: function() {
			$('li.newcomment, tr.newcomment').each(function() {
				if (P3.utility.isElementVisible(this)) {
		...
		toggleUpdates: function( updater ) {
			switch (updater) {
				case "unewposts":
					if (0 == getPostsUpdate) {
						getPostsUpdate = setInterval(P3.posts.fetch, updateRate);
					}
					else {
				...
				case "unewcomments":
					if (0 == getCommentsUpdate) {
						getCommentsUpdate = setInterval(P3.comment.fetch, updateRate);
					}
					else {
	...
	if ((inlineEditPosts || inlineEditComments ) && isUserLoggedIn) {
		$.editable.addInputType('autogrow', {
			...
			plugin : function(settings, original) {
				window.P3.utility.autoGrowTextarea($('textarea', this));
			}
		});
	}
