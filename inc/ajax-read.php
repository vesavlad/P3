...
class P3Ajax_Read {
	public static function dispatch() {
		...
		if ( is_callable( array( 'P3Ajax_Read', $action ) ) ) {
			status_header( 200 );
			call_user_func( array( 'P3Ajax_Read', $action ) );
		} else {
			die( '-1' );
		}
		exit;
	 ...
	 */
	public static function logged_in_out() {
	...
	public static function tag_search() {
		...
		$tags = array();
		$results = $wpdb->get_results( "SELECT name, count FROM $wpdb->term_taxonomy AS tt INNER JOIN $wpdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = 'post_tag' AND t.name LIKE ( '%". like_escape( esc_sql( $term ) ) . "%' ) ORDER BY count DESC" );
		...
		if ($number_of_new_comments > 0) {
			foreach ($comments as $comment) {
				...
				if ( in_array( $comment->comment_post_ID, $visible_posts ) ) {
					ob_start();
					P3_comments($comment, array( 'max_depth' => $max_depth, 'before' => ' | ' ), $depth );
				...
				$comment_widget_html = '';
				if ( $lc_widget )
					$comment_widget_html = P3_Recent_Comments::single_comment_html( $comment, $avatar_size );
	...
	public static function new_comment() {
		...
		if ( get_option( 'require_name_email' ) && !$user->ID )
			if ( strlen( $comment_author_email ) < 6 || '' == $comment_author ) {
				die( '<p>'.__( 'Error: please fill the required fields (name, email).', 'P3' ).'</p>' );
			} elseif ( !is_email( $comment_author_email ) ) {
			    die( '<p>'.__( 'Error: please enter a valid email address.', 'P3' ).'</p>' );
			}
		...
		else echo __("Error: Unknown error occurred. Comment not posted.", 'P3' );
	}
}
